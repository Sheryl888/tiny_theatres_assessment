...
where (select ROUND(min(t.ticket_price),2)
from tickets)
...
from customer c
join tickets t using (customer_id)
join `show` s using (show_id)
...
truncate(sum(t.ticket_price),2) total_ticket_revenue  
from `show` s
join tickets t using (show_id)
...
from customer c
join tickets t using (customer_id)
-- where (select max(sum(t.ticket_price)) from ticket t) 
group by concat(c.first_name, " ", c.last_name)
